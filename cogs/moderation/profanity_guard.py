...
class ProfanityGuard(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if lvl == 1:
            await self.log(message.guild, f"{note} ⚠️ Figyelmeztetés.")
        elif lvl == 2:
            minutes = max(1, self.to_min_l2)
            try:
                await member.timeout(timedelta(minutes=minutes), reason="Profanity L2")
                await self.log(message.guild, f"{note} ⛔ Timeout {minutes} perc.")
            except Exception:
                await self.log(message.guild, f"{note} (L2) — timeout sikertelen.")
        elif lvl == 3:
            ...
            try:
                ...
                await self.log(message.guild, f"{note} (L3) — timeout sikertelen.")
        else:
