...
# Moderation / ticket config
LANG_HINT_EVERY = int(os.getenv("LANG_HINT_EVERY", "0"))  # 0 disables language nagging
...
# Human-chat boost
WAKE_WORDS = tuple(w.strip().lower() for w in os.getenv("WAKE_WORDS", "isero,x").split(",")) if os.getenv("WAKE_WORDS", "").strip() != "" else ("isero","x")
ALLOW_STAFF_FREESPEECH = os.getenv("ALLOW_STAFF_FREESPEECH", "true").lower() in ("1","true","yes")
